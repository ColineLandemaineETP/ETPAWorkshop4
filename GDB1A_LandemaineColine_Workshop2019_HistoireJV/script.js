...
function init(){
	...
	var nes;
	var victory;
	var falseVictory; //ne me tuez pas svp
	var nes2;
}
...
function preload(){
    ...
    this.load.image('objective','assets/nes.png');
    this.load.image('victory','assets/victory.png');
    this.load.image('falseVictory','assets/falseVictory.png');
...
function create(){
	...
	platforms.create(1200,200,'sol').setScale(0.50).refreshBody();
	platforms.create(200,390,'sol').setScale(0.50).refreshBody();
	platforms.create(925,650,'short').setScale(0.35).refreshBody();
	platforms.create(700,520,'short').setScale(0.35).refreshBody();
	platforms.create(690,290,'short2').setScale(0.35).refreshBody();
    ...
    nes = this.physics.add.sprite(1075,121,'objective');    //en violet (st): coord.
    nes2 = this.physics.add.sprite(200,280,'objective');

    //emplacement des poisons

    poisons = this.physics.add.group();
    var poison = poisons.create(945, 150, 'poison');
    var poison2 = poisons.create(680, 350, 'poison');
    ...
    this.physics.add.collider(nes, platforms);
    this.physics.add.collider(player, nes, hitNes, null, this);

    this.physics.add.collider(nes2, platforms);
    this.physics.add.collider(player, nes2, hitNes2, null, this);
...
function hitNes (player, nes){             
   alert("victory\nManque de temps, ici est sensé apparaître une illustration en liens avec la console NES");
   this.add.image(0, 0, 'victory').setOrigin(0, 0);
};

function hitNes2 (player, nes2){             
   alert("falseVictory\nbaited, essayez encore !");
   this.add.image(0, 0, 'falseVictory').setOrigin(0, 0);
};
