...
function preload(){
    ...
    this.load.image('objective','assets/superNin.png');
...
function create(){
	...
	platforms.create(520,400,'sol').setScale(0.35).refreshBody();
	platforms.create(1000,680,'short').setScale(0.35).refreshBody();
	platforms.create(1090,550,'short2').setScale(0.35).refreshBody();
	platforms.create(950,425,'short2').setScale(0.35).refreshBody();
	platforms.create(800,290,'short').setScale(0.35).refreshBody();
    ...
    superNin = this.physics.add.sprite(500,165,'objective');    //en violet (st): coord.
    
    //emplacement des poisons

    poisons = this.physics.add.group();
    var poison = poisons.create(690, 352, 'poison');
    var poison2 = poisons.create(410, 352, 'poison');
    var poison3 = poisons.create(325, 352, 'poison');
    var poison4 = poisons.create(610, 352, 'poison');
    ...
    this.physics.add.collider(superNin, platforms);
    this.physics.add.collider(player, superNin, hitSuperNin, null, this);
...
function hitSuperNin (player, superNin){             
   alert("victory\nManque de temps, ici est sensé apparaître une illustration en liens avec la console Super Nintedo");
